...
namespace Tile_Engine
{
	class SpriteAnimation
	{
		...
		// True if aura texture is around 
		public bool baura = true;
		...
		public SpriteAnimation(string ID, Texture2D Texture, Texture2D aura = null )
		{
		...
		{
			float aurascale = 2.0f;

			// Draw sprites if they are animate
			if (bAnimating)
			{
				...
				// Don't draw an aura that doesn't exist
				if (aura != null)
					spriteBatch.Draw(aura,
								camera.WorldtoScreen(v2Position) + v2Center +
										new Vector2(
										((int)DrawOffset.X) - ((aurascale * aura.Width) / 2),
										((int)DrawOffset.Y) - ((aurascale * aura.Height) / 2)
									),
									null,
									Color.White,
									0.0f,
